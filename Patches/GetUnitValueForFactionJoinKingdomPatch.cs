...
namespace AllegianceOverhaul.Patches
{
  ...
  public class GetUnitValueForFactionJoinKingdomPatch
  {
    public static void Postfix(IFaction factionForEvaluation, int __result, JoinKingdomAsClanBarterable __instance)
    {
      ...
      if (!SettingsHelper.FactionInScope(factionForEvaluation, Settings.Instance.EnsuredLoyaltyDebugScope) && !SettingsHelper.FactionInScope((IFaction)iOriginalOwnerClan, Settings.Instance.EnsuredLoyaltyDebugScope))
        return;
